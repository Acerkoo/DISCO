...
def test_batch_prediction_acc(model, tok, hparams, prompts, target, device, locality=False):
    ...
    with torch.no_grad():
        ...
        return textual_ans, textual_target, np.mean(np.equal(ans, target))
...
def test_seq2seq_batch_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False):
    ...
    with torch.no_grad():
        ...
        return ans[:, :-1], trg_tok['input_ids'][:, :-1], \
            torch.mean((trg_tok['input_ids'][:,:-1] == ans[:,:-1]).float(), dim=-1).detach().cpu().numpy().tolist()
...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False):
    ...
    with torch.no_grad():
        ...
        textual_answers, textual_targets, res = [], [], []
        if not isinstance(answers[0], list):
            answers, labels = [answers, ], [labels,] 

        ans_probs, targ_probs = [], []
        # print("logits.shape: {}".format(logits.shape))
        # print("answers.shape: {}".format(len(answers[0])))
        # print("target.shape: {}".format(len(labels[0])))
        logits = logits.softmax(dim=-1).cpu()

        for idx, (ans, label) in enumerate(zip(answers, labels)):
            temp_acc = np.mean(np.equal(ans, label))
            res.append(temp_acc)

            textual_ans = tok.decode(ans, skip_special_tokens=True).strip()
            textual_label = tok.decode(label, skip_special_tokens=True).strip()

            textual_answers.append(textual_ans)
            textual_targets.append(textual_label)

            length = len(ans)
            temp_logits = logits[idx][-length-1:-1]
            ans_prob = [round(v, 4) for v in temp_logits.gather(-1, torch.tensor(ans).unsqueeze(-1)).squeeze(-1).tolist()]
            targ_prob = [round(v, 4) for v in temp_logits.gather(-1, torch.tensor(label).unsqueeze(-1)).squeeze(-1).tolist()]

            ans_probs.append(ans_prob)
            targ_probs.append(targ_prob)

        if locality:
            return answers[0], textual_answers[0], ans_probs[0]
        else:
            return textual_answers[0], textual_targets[0], answers[0], labels[0], res[0], ans_probs[0], targ_probs[0]
