...
class BaseEditor:
    ...
    def edit(self,
        ...
        all_metrics = []
        for i, request in enumerate(requests):
            if self.alg_name == 'IKE' or self.alg_name == "DISCO":
                assert 'train_ds' in kwargs.keys() or print('IKE need train_ds(For getting In-Context prompt)')
                metrics = {
                    "pre": compute_icl_edit_quality(self.model, self.model_name, self.hparams, self.tok, self.apply_algo,
        ...
        for i, request in enumerate(requests):
            ...
            if self.alg_name == 'IKE' or self.alg_name == "DISCO":
                assert 'train_ds' in kwargs.keys() or print('IKE/DISCO need train_ds(For getting In-Context prompt)')
                ...
                all_metrics[i].update({
                    ...
                    "requested_rewrite": request,
                    "time": exec_time,
                    "post": compute_icl_edit_quality(self.model, self.model_name, self.hparams, self.tok, self.apply_algo,
                                                     request, self.hparams.device, kwargs["train_ds"]),
                })
                ...
                if 'locality' in all_metrics[i]['post'].keys():
                    for locality_key in request['locality'].keys():
                        assert len(all_metrics[i]['post']['locality'][f'{locality_key}_output']['ans']) == \
                               len(all_metrics[i]['pre']['locality'][f'{locality_key}_output']['ans'])
                        locality_result = []
                        for ans,label in zip(all_metrics[i]['post']['locality'][f'{locality_key}_output']['ans'], \
                                             all_metrics[i]['pre']['locality'][f'{locality_key}_output']['ans']):
                            locality_result.append(np.mean(np.equal(ans, label)))
                        all_metrics[i]['post']['locality'][f'{locality_key}_acc'] = {
                            "post": all_metrics[i]['post']['locality'][f'{locality_key}_output']['textual'],
                            "pre": all_metrics[i]['pre']['locality'][f'{locality_key}_output']['textual'],
                            "acc": locality_result,
                            "post_prob": all_metrics[i]['post']['locality'][f'{locality_key}_output']['prob'],
                            "pre_prob": all_metrics[i]['pre']['locality'][f'{locality_key}_output']['prob'],
                        }
    ...
    def _prepare_requests(self,
        ...
        for prompt, ground_truth_, target_new_ in \
            zip(prompts, ground_truth, target_new)
        ]
        ...
        if portability_inputs is not None:
            for portability_key in portability_inputs.keys():
                ...
                for i, request in enumerate(requests):
                    if "sub_question" in portability_inputs[portability_key]:
                        request['portability'].update(
                            {
                                portability_key: {
                                    ...
                                    "first_question": portability_inputs[portability_key]["sub_question"][0][i],
                                    "first_ans": target_new[i],
                                    "second_question": portability_inputs[portability_key]["sub_question"][1][i],
                                    "second_ans": portability_inputs[portability_key]['ground_truth'][i],
                                }
                            }
                        )
                    else:
